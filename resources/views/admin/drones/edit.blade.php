<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="font-extrabold text-2xl text-white tracking-tight">Edit Drone: {{ $drone->name }}</h1>
                    <p class="text-xs text-slate-400 mt-0.5">Perbarui informasi spesifikasi, tarif sewa, atau status publikasi</p>
                </div>
            </div>

            <a href="{{ route('drones.show', $drone->slug) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-[#38BDF8] bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 transition">
    ...
    <div class="py-8 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('admin.drones.update', $drone) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 rounded-2xl bg-[#0F172A]/90 border border-slate-800/90 shadow-xl backdrop-blur space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name', $drone->name) }}" placeholder="Contoh: DJI Mavic 3 Pro"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Jelaskan keunggulan kamera, stabilisasi, dan peruntukan unit drone ini..."
                              class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">{{ old('description', $drone->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-[#0F172A]/90 border border-slate-800/90 shadow-xl backdrop-blur space-y-4">
                ...
                <div class="grid sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="camera" value="{{ old('camera', $drone->camera) }}" placeholder="4K/60fps Hasselblad"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Waktu Terbang (Menit)</label>
                        <input type="number" name="flight_time_min" value="{{ old('flight_time_min', $drone->flight_time_min) }}" placeholder="43"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Bobot / Berat (Gram)</label>
                        <input type="number" name="weight_g" value="{{ old('weight_g', $drone->weight_g) }}" placeholder="249"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-[#0F172A]/90 border border-slate-800/90 shadow-xl backdrop-blur space-y-4">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="1000" name="daily_rate" required value="{{ old('daily_rate', $drone->daily_rate) }}" placeholder="350000"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Tarif Mingguan (Rp)</label>
                        <input type="number" step="1000" name="weekly_rate" value="{{ old('weekly_rate', $drone->weekly_rate) }}" placeholder="1750000"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Tarif Jasa Pilot / Hari (Rp)</label>
                        <input type="number" step="1000" name="pilot_daily_rate" value="{{ old('pilot_daily_rate', $drone->pilot_daily_rate) }}" placeholder="500000"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Biaya Antar-Jemput (Rp)</label>
                        <input type="number" step="1000" name="delivery_fee" value="{{ old('delivery_fee', $drone->delivery_fee) }}" placeholder="50000"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Nilai Penggantian Unit (Rp)</label>
                        <input type="number" step="1000" name="replacement_value" value="{{ old('replacement_value', $drone->replacement_value) }}" placeholder="15000000"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Jumlah Stok Unit Tersedia</label>
                        <input type="number" name="stock" min="1" max="99" value="{{ old('stock', $drone->stock) }}"
                               class="w-full rounded-xl bg-[#0B1220] border border-slate-700 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-[#38BDF8] focus:ring-1 focus:ring-[#38BDF8]">
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-[#0F172A]/90 border border-slate-800/90 shadow-xl backdrop-blur space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-1.5">Ganti Foto Drone (JPG / PNG, Maksimal 2MB)</label>
                    ...
                    @if ($drone->image_path)
                        <div class="mt-3 flex items-center gap-3 p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                            <img src="{{ asset('storage/'.$drone->image_path) }}" alt="{{ $drone->name }}" class="h-16 w-16 object-cover rounded-lg border border-slate-700">
                            <div>
                                <span class="text-xs text-slate-300 font-medium block">Foto Saat Ini</span>
                                <span class="text-[11px] text-slate-500 font-mono">{{ $drone->image_path }}</span>
                            </div>
                        </div>
                    @endif
                </div>
                ...
                <div class="pt-2">
                    <label class="inline-flex items-center gap-2.5 text-sm text-slate-200 cursor-pointer">
                        ...
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $drone->is_active)) class="rounded border-slate-700 bg-slate-900 text-[#38BDF8] focus:ring-[#38BDF8]">
